---
title: "Petrópolis em 3 Dias"
destination: "Petrópolis"
origin: "Niterói"
base: "Loft no Centro"

logistics:
  route:
    - "Ponte Rio-Niterói"
    - "Linha Vermelha"
    - "BR-040"
  time: "1h30 a 2h"
  tips:
    - "Sair antes de 8h ou após 10h"
    - "Evitar pico na ponte"
    - "Tanque cheio antes da serra"

days:
  - day: 1
    title: "Centro histórico (leve e estratégico)"
    focus: "Chegar + adaptação + caminhada"
    events:
      - time: "Tarde"
        desc: "Museu Imperial, Palácio de Cristal, Catedral São Pedro de Alcântara"
        icon: "🏛️"
      - time: "Noite"
        desc: "Jantar no centro, caminhada leve"
        icon: "🌙"

  - day: 2
    title: "Experiência completa (natureza + serra)"
    focus: "O carro faz diferença aqui"
    events:
      - time: "Manhã"
        desc: "Parque Nacional da Serra dos Órgãos (trilha leve ou mirante)"
        icon: "⛰️"
      - time: "Almoço"
        desc: "Cervejaria Bohemia"
        icon: "🍺"
      - time: "Tarde"
        desc: "Casa de Santos Dumont, ruas históricas + cafés"
        icon: "☕"
      - time: "Noite"
        desc: "Barzinho leve ou descanso"
        icon: "🍷"

  - day: 3
    title: "Chill + Retorno inteligente"
    focus: "Fechar sem pressa"
    events:
      - time: "Manhã"
        desc: "Café tranquilo, comprar lembranças"
        icon: "🛍️"
      - time: "Opcional"
        desc: "Palácio Quitandinha"
        icon: "🏰"
      - time: "Retorno"
        desc: "Sair antes das 16h (evita trânsito)"
        icon: "🚗"

images:
  - "https://images.unsplash.com/photo-1590059533350-01053bb1082d?auto=format&fit=crop&w=1920&q=80"
  - "https://images.unsplash.com/photo-1628178385038-f86a928926eb?auto=format&fit=crop&w=1920&q=80"
  - "https://images.unsplash.com/photo-1543884814-c48c34fbc875?auto=format&fit=crop&w=1920&q=80"
---

# 🏔️ Roteiro de Viagem: Petrópolis (3 dias)

![Vista da serra](https://images.unsplash.com/photo-1628178385038-f86a928926eb?auto=format&fit=crop&w=1920&q=80)

## 📍 Informações Gerais

- **Destino:** Petrópolis  
- **Origem:** Niterói  
- **Base:** Loft no Centro  

---

## 🚗 Logística de deslocamento

- **Rota:** Ponte Rio-Niterói → Linha Vermelha → BR-040  
- **Tempo estimado:** 1h30 a 2h  

### ⚠️ Dicas

- Sair cedo (antes de 8h ou depois de 10h)  
- Evitar pico na ponte  
- Tanque cheio antes da serra  

---

## 🗓️ Dia 1: Centro histórico

**Foco:** Chegar + adaptação + caminhada  

| Horário | Atividade |
|--------|----------|
| 🕒 Tarde | Museu Imperial, Palácio de Cristal, Catedral |
| 🌙 Noite | Jantar no centro, caminhada leve |

---

## 🌄 Dia 2: Experiência completa

**Foco:** Natureza + carro  

| Horário | Atividade |
|--------|----------|
| ☀️ Manhã | Parque Nacional da Serra dos Órgãos |
| 🍽️ Almoço | Cervejaria Bohemia |
| 🌤️ Tarde | Casa de Santos Dumont + cafés |
| 🌙 Noite | Barzinho leve |

![Cervejaria](https://images.unsplash.com/photo-1543884814-c48c34fbc875?auto=format&fit=crop&w=1920&q=80)

---

## 🌿 Dia 3: Chill + retorno

**Foco:** Sem pressa  

| Horário | Atividade |
|--------|----------|
| ☕ Manhã | Café + lembranças |
| 🏛️ Opcional | Palácio Quitandinha |
| 🚗 Retorno | Antes das 16h |

---

## 🧠 Insight

> Petrópolis tem dois modos: turista básico e explorador.  
> Esse roteiro te coloca no modo **explorador total**, usando o centro como base e o carro como vantagem estratégica.
